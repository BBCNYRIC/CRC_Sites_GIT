...
<!--- do a check first to make sure the teacher owns this page --->
...
WHERE tid = <cfqueryparam value="#FORM.tid#" cfsqltype="CF_SQL_INTEGER">
	AND (owner = <cfqueryparam value="#session.auth.loggedInId#" cfsqltype="CF_SQL_INTEGER">
		OR tid = <cfqueryparam value="#session.auth.loggedInId#" cfsqltype="CF_SQL_INTEGER">)
</cfquery>
...
UPDATE contacts
SET description = <cfqueryparam value="#Trim(preserveSingleQuotes(FORM.description))#" cfsqltype="CF_SQL_CHAR">,
...
WHERE contacts.tid = <cfqueryparam value="#FORM.tid#" cfsqltype="CF_SQL_INTEGER">
	...
	AND (teacher.owner = <cfqueryparam value="#session.auth.loggedInId#" cfsqltype="CF_SQL_INTEGER">
		OR teacher.tid = <cfqueryparam value="#session.auth.loggedInId#" cfsqltype="CF_SQL_INTEGER">)
</cfquery>
...
Stop showing after: <cfinput type="Text" name="showDateTo" value="#DateFormat(getuserinfo.showDateTo,'M/D/YYYY')#" validate="date" required="No" size="16" id="showDateTo" placeholder="Click to choose date">
		   ...
		   <cfif FORM.tid NEQ session.auth.loggedInId>
		  <tr> 
            <td align="right"> <a href="trempage.cfm?teacher_id=<cfoutput>#getuserinfo.tid#</cfoutput>">Delete this page</a> </td>
          </tr>
		  </cfif>
